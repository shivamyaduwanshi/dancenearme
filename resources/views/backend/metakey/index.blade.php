...
@section('title') {{__('Meta List')}} @endsection

@section('content')
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">{{__('Meta List')}} ({{$metakeys->total()}})</h3>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-12 text-right">
        <a href="{{route('backend.create.metakey')}}" class="btn btn-primary">{{__('Add Meta')}}</a>
    </div>
</div>
<br>
<div class="row">
    <div class="col-lg-12">
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                ...
                <tbody>
                    @php $i = $metakeys->perPage() * ($metakeys->currentPage() - 1); @endphp
                    @foreach ($metakeys as $key => $value)
                        <tr class="odd gradeX">
                            ...
                            <td>{{$value->value ?? '-'}}</td>
                            <td>
                                <a href="{{route('backend.edit.metakey',$value->id)}}" class="btn btn-sm btn-primary">{{__('Info')}}</a>
 ...
 <div class="modal fade" id="deleteModal" role="dialog">
    <div class="modal-dialog modal-md">
        <form class="form" action="{{route('backend.delete.metakey')}}" method="POST">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">{{__('Delete Meta')}}</h4>
        </div>
        <div class="modal-body">
                @csrf
                {{ method_field('DELETE') }}
                 <input type="hidden" name="id" value="">
                 <p>{{__('Are you sure want to delete meta?')}}</p>
        </div>
