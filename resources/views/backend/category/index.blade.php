...
@section('title') {{__('Dances List')}} @endsection

@section('content')
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">{{__('Dances')}} ({{$categories->total()}})</h3>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-12 text-right">
        <a href="{{route('backend.create.category')}}" class="btn btn-primary">{{__('Add Dance')}}</a>
    </div>
</div>
<br/>
<div class="row">
    ...
    <div class="modal-dialog modal-md">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">{{__('Delete Dance Type')}}</h4>
        </div>
        <div class="modal-body">
                @csrf
                {{ method_field('DELETE') }}
                 <input type="hidden" name="id" value="">
                 <p>{{__('Are you sure want to delete this dance type?')}}</p>
        </div>
